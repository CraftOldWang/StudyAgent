...
2026-09-10 入口修正（覆盖旧版学习准备约定）：学习大纲与学习对话分开。大纲和历史会话由服务端列表读取，无编号输入。大纲只在用户明确新建时生成，知识点数量由课件内容确定；课件/习题角色只用于规划输入。大纲按章节展示知识点及子主题，标记未开始/学习中/已完成；“开始学习”首次建立会话，之后“继续学习”复用同一会话与进度。离开对话查看大纲不重新生成，历史会话点击加载已保存消息。旧版直接创建的会话也可在独立大纲页查看保存的计划。
