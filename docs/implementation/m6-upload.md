...
6. 浏览器 Worker 增量 SHA-256 与上传客户端已接入，默认 8 MiB/4 并发，展示字节进度、校验/合并状态及可恢复错误；暂停后查询缺失分片继续，不把前端断开当成服务器取消成功。
...
[正式 400 MB 对照](m6-upload-performance-report.md) 已完成 20/20 成功样本。同为 4 并发时，完整耗时中位数由 35.048 s 降至 17.833 s，减少 49.1%；不将这些数字外推为弱网指标。

[浏览器验收](../evidence/m6/upload-ui-v1.json) 使用独立库中的 400 MB 合成文件，验证哈希和传输暂停、跨资料库切换保持原目标、页面刷新后自动读取已保存进度、重新选择同一文件继续、完成后出现文档。浏览器显示 312 MiB 时暂停，服务端另完成一个在途分片，最终保存 40 片共 335,544,320 字节；刷新显示 320 MiB，缺少的八片共 64,455,680 字节。后续同一会话完成 48 片。这个剩余字节数依据服务器缺片列表计算，不冒充浏览器抓取的实际网络流量。

34 项前端回归、类型检查、生产构建与 premium strict 审计通过（0 findings）。浏览器验证桌面和390px布局：根 clientWidth/scrollWidth 均380，上传区域约358，无横向溢出；恢复默认视口后保留验收标签页。真实验收发现并修正刷新进度先显示0、Java Long字节数返回字符串、文档UTC时间被当成本地时间的问题。首次失败的旧上传API测试和测试钩子返回mock被当作清理函数的问题均保留日志，最终测试通过。

Worker 使用固定版本 `@noble/hashes@2.4.0`，每次读取8MiB，增量结果与Node原生SHA-256跨块对照一致；文件内容不进入localStorage。参考 [库的增量接口](https://github.com/paulmillr/noble-hashes)。界面层只保存原资料库、文件元数据、哈希和sessionId，返回页面会先查服务端；不会以刷新为理由自动发送模型请求。

## 尚未覆盖的边界

真实应用重启目前发生在分片完成后、合并开始前；合并响应丢失和VERIFIED阶段恢复由定向测试覆盖，尚未用真实进程终止命中这两个短窗口。不实现后台定时清理所有历史遗留对象，取消只清理指定上传。课程环境尚未重新部署V12/V13，本轮均为独立上传数据库，不把该环境的上传成功计作资料解析或自然学习验收。
