...
**授权更新：用户已明确表示允许这些下载课程资料外传至DeepSeek，因为资料没有隐私信息。此前拒绝作为历史记录保留；下一步优先恢复真实课程验收，自编材料不替代正式实验。**

此前真实课程→DeepSeek的自动审批拒绝期间，为独立检查实现，另建完全自编虚构课程`eval/synthetic-learning/`，固定“键与作用域、TTL、版本、淘汰、重复请求”五个单元及四道习题。它不含下载课程，不替代两门真实课程的正式压缩实验。
