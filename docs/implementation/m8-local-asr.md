...
后端17项定向测试与222项完整测试通过（219通过、3项既有跳过），前端37项测试、构建与严格UI审计通过。真实音视频的转写、入库、检索及后端终止恢复已实测；2026-09-09两条真实学习问答与来源复验也已通过，详见下节。用户已明确授权课程资料外传至DeepSeek。本地ASR尝试另计，报告音频时长、处理耗时和RTF，不虚构云端token usage。

### 真实学习与来源复验（2026-09-09）

使用`scripts/verify-asr-learning.py`复用已INDEXED的完整录音与60秒视频，分别经规划API生成一个知识点，再经自然消息API讲解、追问。录音会话2097599275307757570讨论支配与直接支配者，视频会话2097599248690704385讨论希尔排序与间隔分组。四回合均成功，讲解推进到EXPLAINING、追问不推进，重放requestId保持原回合与attemptCount。两会话各6次学习/摘要尝试，累计输入分别39862/19372，usage完整；规划用量单独记账。

首轮模型缩写chunkId导致来源验收失败，原结果保留。系统提示明确要求完整标识后新建会话复验，所有回答引用均经生产来源API查询并核对转写时间戳；视频浏览器恢复两回合历史并打开MP4来源抽屉。26项规划/对话/摘要相关回归通过。证据`docs/evidence/m8/learning-v1-citation-failure.json`、`learning-v2-review.json`和`docs/evidence/m5/citation-build.json`。验收覆盖两条有来源问答路径，不代表全部教学内容正确或ASR准确率；转写同音字问题仍需用户结合原资料判断。
