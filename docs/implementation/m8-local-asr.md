...
后端17项定向测试与222项完整测试通过（219通过、3项既有跳过），前端37项测试、构建与严格UI审计通过。真实音视频的转写、入库、检索及后端终止恢复已实测；学习问答仍受M5所记录的DeepSeek课程内容外传审批阻塞，不能据此关闭M8。本地ASR尝试另计，报告音频时长、处理耗时和RTF，不虚构云端token usage。
