...
视觉参考是备考时并排摆放的课程目录与批注笔记。用户需要知道学什么、为什么优先、当前做到哪一步，页面围绕实际课程与当前知识点展开。工作台采用紧凑课程导航、可展开的重点计划和连续对话，不设置宣传式 hero 或虚构学习统计。

面向使用中文界面的大学计算机课程学习者，课程正文可为中英混合；这是产品工具界面，非营销页面。主要使用场景为桌面长时间阅读，窄屏可查看进度、回答问题与翻阅卡片。当前只提供浅色主题，无日本市场专属业务假设。

唯一醒目的表达是知识点路线旁的琥珀重点批注：优先级必须来自已保存计划，完成标记必须来自后端状态；不能把“已完成”美化成已掌握。其余位置保持安静，留出正文阅读宽度。

用户已授权修改现有界面。本次从旧深绿侧栏、衬线大标题与浅奶油卡片，调整为冷灰纸面、墨蓝正文与深青操作色。保留“知识库/学习”工作区区分，减少大块装饰、胶囊标签和多层阴影。前端显示名采用用户项目名 StudyPilot，仓库与后端包名保留 StudyAgent。
...
桌面以 248px 课程侧栏加主内容区，最大阅读工作区 1216px。计划与对话并列，计划可折叠；对话和资料表各自具备明确滚动区域，长表单保持自然文档滚动。所有 grid/flex 可缩小项显式 min-width:0。

780px 以下侧栏变为紧凑顶部课程导航，知识点计划可展开，聊天与输入依次布局。文档表允许其自身横向滚动，不能撑开整个页面。390px 下根文档不应有水平溢出。常规内容间距 24px，控制间距 8px；窄屏缩小外边距，不压缩触摸目标。
...
控件圆角 10px，细节 6px，内容面板 14px；状态点、知识点编号可为圆形，但普通段落与按钮不全部胶囊化。重点条目使用左侧短标记而非巨大色块。
