...
视觉参考是备考时并排摆放的课程目录与批注笔记。用户需要知道学什么、为什么优先、当前做到哪一步，页面围绕实际课程与当前知识点展开。工作台采用紧凑课程导航、独立的大纲管理页和连续对话，不设置宣传式 hero 或虚构学习统计。
...
用户已授权修改现有界面。本次从旧深绿侧栏、衬线大标题与浅奶油卡片，调整为冷灰纸面、墨蓝正文与深青操作色。采用“知识库/学习大纲/学习对话”三个入口，减少大块装饰、胶囊标签和多层阴影。前端显示名采用用户项目名 StudyPilot，仓库与后端包名保留 StudyAgent。
...
桌面以 248px 课程侧栏加主内容区，最大阅读工作区 1216px。应用固定为视口高度；资料库列表独立滚动，品牌和创建入口保持可见。知识点计划默认收起，通过顶部“学习计划”按钮展开为 220px 辅助栏，并独立滚动。
...
学习大纲页采用可滚动的大纲列表与正文；默认读取已保存大纲，只有显式“新建学习大纲”才显示资料选择。章节、知识点、子主题按层级展示，完成状态取自绑定会话，不在浏览器猜测或重置。对话页首先列出当前资料库的历史会话，点击进入，标题、日期和进度取代手输编号。会话返回大纲时读取最新进度；保持已有聊天滚动和固定输入区。
