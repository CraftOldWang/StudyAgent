...
export function CardDrafts({ cards, busy, confirming, onSave, onConfirm, onRewrite }: {
  ...
  return <section className="cards-block" aria-label="待确认复习卡片">
    ...
    <p className="muted">可直接编辑，或通过对话让我重写。确认全部卡片后写入 Anki，再进入下一知识点。</p>
    ...
    <div className="action-row">
      ...
      <button type="button" disabled={busy || invalid} onClick={() => void onConfirm(drafts)}>{busy ? '正在处理…' : confirming ? '重试写入并继续' : '确认全部卡片并写入 Anki'}</button>
    </div>
  </section>
}
