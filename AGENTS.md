...
- 三份当前设计文档之间若存在冲突或多个可辩护答案，按 §1 的决策边界处理：触及高影响边界或没有明确推荐时必须停下来交给用户决定；否则可以按明确推荐执行并留痕。不得用旧决策、文档日期或 agent 自己的偏好裁决。
...
以下完成门槛保持不变，但统一延后到完整模块或当前明确批次的实现边界执行：实现过程中应编写必要的测试代码，不在模块或批次实现结束前运行 `compile`、测试或独立验收，避免对每个小任务重复执行验证。模块或批次实现完整后，统一运行相关测试、`mvn compile`、必要的全量 `mvn test`，并由未参与相关实现的 subagent 独立验收。

批次验收前，相关任务只能标记为“实现完成/待验收”，不得关闭对应 Issue，也不得将 Project 状态设为 Completed。测试失败只阻止受影响任务标记最终完成，不阻止相互独立模块继续实现。
