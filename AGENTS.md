...
- 每个稳定工作流由一个长期复用的实现 agent 负责。实现 agent 拥有该工作流的仓库勘察、代码与文档编辑，并在完整模块或明确批次结束后统一编译和测试；后续 agent 只按难度选择 `gpt-5.6-sol` 的 `low`/`medium` 或 `gpt-5.6-terra`，不新开 `gpt-6-astra`。
- 模块 verifier 必须是未参与该模块实现的独立 agent，在上述允许模型范围内按审查难度选择。它异步审查提交和关键逻辑，默认不重复运行实现者已经通过的整套测试；只有高风险、证据不足或结果可疑时才补跑相关测试。
- 当前主模型由用户自行切换。agent 不自动调用 `sol-pro-browser-delegate`；只有用户明确指定浏览器 Pro 委派时才执行，并必须确认网页端存在且选中用户指定的可见模型选项，不能以不可见后端标识代替确认。
