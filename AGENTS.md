...
- 每个稳定工作流由一个长期复用的 `gpt-5.6-sol`、`medium` 实现 agent 负责。实现 agent 拥有该工作流的仓库勘察、代码与文档编辑，并在完整模块或明确批次结束后统一编译和测试。
- 模块 verifier 必须是未参与该模块实现的独立 `gpt-5.6-sol`、`high` agent。它异步审查提交和关键逻辑，默认不重复运行实现者已经通过的整套测试；只有高风险、证据不足或结果可疑时才补跑相关测试。
