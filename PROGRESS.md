...
- [~] M6：重点计划/完整历史/自然消息 SSE/来源抽屉已接入；26项前端回归、3项后端定向测试、类型与构建、设计 lint/23个 token 对照/严格静态审计通过。真实浏览器验证旧计划展示、合成会话三卡恢复、创建改名、错误保留、键盘与390px布局；来源生产API验证归属隔离，模型调用零增量。见 `docs/implementation/m6-core-ui.md`。原上传JAR与计时补丁已冻结，14项上传测试通过；400,000,000字节、8MiB分片、串行/4并发各5次全部成功，总耗时中位数41.103s/35.048s。独立上传库停用解析消费，12个含烟测文档均STORED、零chunk，账本仍394。见 `docs/implementation/m6-upload.md`。原生分片上传/续传/并发去重、正式压缩/上传对照报告及真实模型流程故障验收待完成。
