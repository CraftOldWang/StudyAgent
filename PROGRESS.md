...
- [~] M4：阶段可恢复入库、embedding 产物复用、逐项索引确认、执行令牌/租约与过期恢复已实现；真实 ES 写入故障及终止容器后的自动恢复通过，未知 usage 保留。该批报告合计176项、0 failure/error、3 skipped（一次旧映射失败后4项复验通过）；父块去重、4096正文预算和生产API四模式另30项回归、8次真实请求通过。18份课程资料与100题已冻结，六种配置120次开发检索完成，结构800/0、parent2400/0已选定。独立验证索引入库及正式RAG/回答评分正在进行；见 `docs/implementation/m4-ingest-recovery.md`、`docs/implementation/m4-chunk-selection.md`。
...
**下一步：** M4正式RAG组件评测已完成：320次独立检索、80次共享排名对照、320次回答/评审；60道可回答验证题中BM25 Hit@5为70%，VECTOR/RRF均91.7%。父块没有总体改善，自动回答评审存在已抽查确认的漏判，不把其通过率当作准确率。报告见 `docs/implementation/m4-rag-validation-report.md`。当前LLM账本382/2000次尝试，两次早期失败usage未知。补测TXT/Markdown时，空文件按预期在PARSING失败，TXT已INDEXED；并发新文档触发CHUNKING写入死锁，InnoDB证据显示idx_document空范围/supremum锁互等，正在修正并复验并发与恢复，M4暂不关闭。随后推进M5规划、自然对话、局部摘要与恢复；摘要/工具全链trace仍需补齐。上传/压缩性能基线未完成前不替换对应实现；Goal尚未完成。
