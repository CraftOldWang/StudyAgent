...
- [~] M4：统一结构父子切块候选已接入真实配置与版本指纹，PDF/PPTX 烟测分别产生 2/6、3/11 个父/子块，仅子块向量化。新增 RustFS 规范文本、embedding 产物缓存、逐项索引确认、执行令牌/租约与过期重新入队；真实 ES 写入故障后通过 API 恢复，6 次 DOCUMENT 调用未增加；真实终止容器后也已由过期扫描自动恢复，保留中断调用的未知 usage。最新测试报告合计 176 项、0 failure/error、3 skipped（全套一次旧映射失败后 4 项定向复验通过），独立打包与启动通过。父块去重/预算、六组以内选型及生产 API RAG 批量评测待做，见 `docs/implementation/m4-ingest-recovery.md`。
...
**下一步：** 继续 M4 父块去重、统一上下文预算、实际命中子块证据与生产 API 检索模式，再冻结课程开发/验证集进行参数选型。embedding usage 已持久化并有真实记录，摘要/工具全链 trace 仍需补齐。随后按 M5 已批准的对话编排替换旧强制工具流程，完成讲解 → 五题 → 三卡和错误恢复；不继续重复同一失败调用碰运气。性能基线完成前不替换对应上传/压缩实现；当前新增阶段尚未全部验收。
