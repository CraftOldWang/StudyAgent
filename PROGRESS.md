...
- [~] M3：已保存优化前源码/参数基线，修正密钥加载、embedding 工作区端点和前端代理，建立独立 eval 环境；新增持久化模型调用账本、2000 次限额、usage 汇总与 API 烟测脚本。当前 Linux 160 项测试报告 0 failure/error、3 skipped；组合命令打包内存不足后，独立重打包成功并通过 JAR 完整性检查。真实 PDF/PPTX 与 provider 已烟测；学习请求仍因未调用状态工具失败，完整 trace 与性能基线待补。实现与证据见 `docs/implementation/m3-runtime-and-measurement.md`。
- [~] M4：已开放 TXT/Markdown/PDF/PPTX 上传并新增 MIME 字段迁移；真实 PPTX 已完成索引及有来源检索。阶段产物复用与失败恢复、统一结构父子切块、六组以内选型及生产 API RAG 评测待做。
...
**下一步：** 真实 embedding/LLM、PDF 与 PPTX 入库检索已通过；完成当前调用计数边界回归并保存检查点。继续补齐 embedding usage、摘要/工具轨迹；推进 M4 的阶段产物与结构打包，同时按 M5 已批准的对话编排移除旧强制工具流程，完成讲解 → 五题 → 三卡和错误恢复。旧学习请求失败证据已保存，不继续重复同一调用碰运气。性能基线完成前不替换对应上传/压缩实现；当前新增阶段尚未全部验收。
