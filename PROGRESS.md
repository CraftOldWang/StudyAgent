...
**下一步：** M4 父块去重、统一正文预算、实际命中子块证据与生产 API 四种检索模式已实现，30 项定向回归及真实 PDF/PPTX 共 8 次模式烟测通过，详见 `docs/implementation/m4-rag-evaluation-interface.md`。继续建立课程资料清单与 100 道问题的开发/冻结验证划分，完成六组以内统一切块选型及正式评分。embedding usage 已持久化并有真实记录，摘要/工具全链 trace 仍需补齐。随后按 M5 已批准的对话编排替换旧强制工具流程，完成讲解 → 五题 → 三卡和错误恢复；不继续重复同一失败调用碰运气。性能基线完成前不替换对应上传/压缩实现；当前新增阶段尚未全部验收。
