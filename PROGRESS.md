...
- [~] M5：已实现持久化规划任务、按文档批次提取、稳定大纲 ID、习题依据与顺序任务、阶段输入指纹/尝试记录/执行权、会话并发创建去重。13项定向回归通过；v1真实链路5次调用完成且重复执行无新增调用，v2验证失败阶段恢复/上游复用并发现数量及重点语义问题，证据如实保留。v3修正已打包，先前真实模型验收被自动审批拒绝；用户现已明确授权课程资料外传，待环境恢复后执行；自然对话、持久化回合、三种摘要策略、失败恢复与 SSE 已实现，28项定向本地测试通过；完整199项测试通过（3项既有跳过），V11迁移与合成数据生产API阶段恢复/并发去重/SSE结果返回通过，模型调用零增量；真实模型与生成中断线验收仍待完成。见 `docs/implementation/m5-planning.md`、`docs/implementation/m5-conversation.md`。
...
- [~] M8：本机faster-whisper/CPU int8转写worker、媒体上传、TRANSCRIBING阶段/续租、转写产物/元数据持久化与复用已实现。后端222项（219通过、3跳过）、前端37项/构建/严格审计通过。完整797.838秒录音转写129.359秒；终止后端后1次ASR+1次缓存复用恢复INDEXED，13次文档embedding。60秒真实MP4经浏览器上传转写15.922秒，来源检索通过；无效媒体及无语音明确失败且无下游产物。390px布局通过。学习问答尚待环境恢复后执行，课程外传已获用户明确授权；见 `docs/implementation/m8-local-asr.md`。
...
**下一步：** 2026-09-09 Docker 已恢复；当前配置 study_agent_eval / Redis DB1 / eval-m5-learning。新增 compiler-phases 文档2097521604013703169已索引。真实规划v3任务2097521725136814081因OUTLINE遗漏候选失败，六次provider调用与失败证据已保留；累计400/2000次。v4修正10项定向测试和打包通过。首次构建遇到内存不足，临时停本项目eval-app/broker/namesrv后构建成功；用户正在考虑提高WSL内存，目前这三个服务仍停止，ASR worker也保持停止。固定用户脚本、用量统计和九副本初始化工具已准备；初始化SQL实库事务验证5点/1计划后回滚，未调用模型。恢复服务后先完成v4真实规划和自然对话/SSE烟测，再冻结两门课的18会话压缩实验；Goal未完成。
