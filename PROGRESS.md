...
- [~] M4：统一结构父子切块候选已接入真实配置与版本指纹，PDF/PPTX 烟测分别产生 2/6、3/11 个父/子块，仅子块向量化。新增 RustFS 规范文本、embedding 产物缓存、逐项索引确认、执行令牌/租约与过期重新入队；真实 ES 写入故障后通过 API 恢复，6 次 DOCUMENT 调用未增加；真实终止容器后也已由过期扫描自动恢复，保留中断调用的未知 usage。该批测试报告合计 176 项、0 failure/error、3 skipped（全套一次旧映射失败后 4 项定向复验通过）。后续父块去重、4096 正文预算与生产 API 四模式已完成，30 项定向回归及 8 次真实请求通过，独立打包与启动通过。六组以内选型及正式 RAG 评分待做，见 `docs/implementation/m4-ingest-recovery.md`。
- [ ] M5：可恢复的大纲 → 重点 → 任务编排、对话主导五题/三卡学习、局部摘要压缩与会话恢复。
...
**下一步：** M4 的 18 份课程原件已全部通过生产 API 入库；100 题已逐条审查并冻结为 20 开发/80 验证，编译原理为留出课程。新增计数 eval 模型入口（1 项服务测试、打包和真实调用通过）及生产 API 评分器（3 项测试通过）；结构 800/80 开发组 20 次请求通过，Hit@5 宏平均 95%，不是最终指标。继续完成六组以内统一切块选型，再运行冻结验证四模式与资料不足回答检查。详见 `docs/implementation/m4-gold-and-evaluation.md`。当前 LLM 账本累计 62/2000 次尝试，含两次 usage 未知的失败，不能将其成本记为零。摘要/工具全链 trace 仍需补齐；随后按 M5 已批准的对话编排替换旧强制工具流程。性能基线完成前不替换对应上传/压缩实现；当前新增阶段尚未全部验收。
